...
# Color constants
COLOR_GRAY = (127, 127, 127)  # RGB for gray
COLOR_YELLOW = (255, 255, 0)  # RGB for yellow
COLOR_GREEN = (0, 255, 0)     # RGB for green
COLOR_CYAN = (0, 255, 255)    # RGB for cyan
COLOR_BLUE = (0, 0, 255)      # RGB for blue
...
class Simulator:
    ...
    def __init__(self, pbf_pin=_DEFAULT_PBF_PIN, go_pin=_DEFAULT_GO_PIN, neopixel_pin=board.NEOPIXEL):
        ...
        self.neopixel_brightness = 0.2
        self.pixels = neopixel.NeoPixel(neopixel_pin, self.neopixel_count, brightness=self.neopixel_brightness)
        #Staus Colors
        self.status_colors = [None] * STATUS_NUM_EVENTS
        self.status_colors[STATUS_UNKNOWN] = COLOR_GRAY
        self.status_colors[STATUS_INITIALIZING] = COLOR_YELLOW
        self.status_colors[STATUS_LAUNCHING] = COLOR_GREEN
        self.status_colors[STATUS_FLOATING] = COLOR_CYAN
        self.status_colors[STATUS_TERMINATING] = COLOR_BLUE

        self._flight_status = STATUS_UNKNOWN
        self.prev_status = STATUS_UNKNOWN
    ...
    def update_neopixel_color(self):
        # Update Neopixel color based on current flight status
        current_status = self._flight_status
        self.pixels.fill(self.status_colors[current_status])
    ...
    def update(self):
        ...
        # Update Neopixel color if status changes
        if self._flight_status != self.prev_status:
            self.update_neopixel_color()
            self.prev_status = self._flight_status

        # Additional check for None status
        if self._flight_status is None or self._flight_status == STATUS_UNKNOWN:
            self.pixels.fill((0, 0, 0))  # Turn off the NeoPixel
        else:
            self.pixels.fill(self.status_colors[self._flight_status])
        ...
        if self.pbf is True and self.streaming is True:
            self.toBlink = False
        else:
            self.toBlink = True

        if self.toBlink:
            # ESTABLISH BLINK INTERVAL
            if self.pbf is False and self.streaming is False:
                self._interval = 5


            if self.pbf is False and self.streaming is True:
                self._interval = 3


            if self.pbf is True and self.streaming is False:
                self._interval = 1


            now = time.monotonic()
            if now > self._last_transition + self._interval:
                self.go = not self.go
